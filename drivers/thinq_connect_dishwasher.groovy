...
metadata {
    definition(name: "ThinQ Connect Dishwasher", namespace: "jonozzz", author: "Ionut Turturica",
               importUrl: "https://raw.githubusercontent.com/jonozzz/hubitat-thinqconnect/refs/heads/main/drivers/thinq_connect_dishwasher.groovy") {
        capability "Sensor"
        capability "Switch"
        ...
        attribute "currentDishWashingCourse", "string"
        attribute "rinseLevel", "string"
        attribute "softeningLevel", "string"
        attribute "doorState", "string"
        attribute "rinseRefill", "string"
        attribute "machineCleanReminder", "string"
        attribute "signalLevel", "string"
        attribute "cleanLightReminder", "string"
...
def processStateData(data) {
    ...
    if (data.operation?.dishWasherOperationMode) {
        def opMode = cleanEnumValue(data.operation.dishWasherOperationMode)
    ...
    if (data.doorStatus?.doorState) {
        def doorState = cleanEnumValue(data.doorStatus.doorState)
        sendEvent(name: "doorState", value: doorState)
        
        // Update contact sensor based on door state
        def contactState = (doorState =~ /(?i)open/ ? "open" : "closed")
        sendEvent(name: "contact", value: contactState)
    }

    // Process dishwasher-specific attributes
    if (data.dishWashingCourse?.currentDishWashingCourse) {
        def course = cleanEnumValue(data.dishWashingCourse.currentDishWashingCourse)
        sendEvent(name: "currentDishWashingCourse", value: course)
    }

    if (data.preference?.rinseLevel) {
        def rinseLevel = cleanEnumValue(data.preference.rinseLevel)
        sendEvent(name: "rinseLevel", value: rinseLevel)
    }

    if (data.preference?.softeningLevel) {
        def softeningLevel = cleanEnumValue(data.preference.softeningLevel)
        sendEvent(name: "softeningLevel", value: softeningLevel)
    }

    // Process binary sensors
    if (data.dishWashingStatus?.rinseRefill != null) {
        def rinseRefill = data.dishWashingStatus.rinseRefill ? "needed" : "ok"
        sendEvent(name: "rinseRefill", value: rinseRefill)
    }

    if (data.preference?.mCReminder != null) {
        def cleanReminder = cleanEnumValue(data.preference?.mCReminder)
        sendEvent(name: "machineCleanReminder", value: cleanReminder)
    }

    if (data.preference?.signalLevel != null) {
        def signalLevel = cleanEnumValue(data.preference?.signalLevel)
        sendEvent(name: "signalLevel", value: signalLevel)
    }

    if (data.preference?.cleanLReminder != null) {
        def lightReminder = cleanEnumValue(data.preference?.cleanLReminder)
        sendEvent(name: "cleanLightReminder", value: lightReminder)
    }
